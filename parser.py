...
async def get_page_data(urls: list):
    # # urls = ['https://terraceramica.ru/product/16443119']
    # urls = ['https://terraceramica.ru/product/16737089', 'https://terraceramica.ru/product/16443119']
    for url in urls:
...
async def gather_data():
    ...
    async with aiohttp.ClientSession(trust_env=True) as session:
        ...
        for link in links3:
            async with aiohttp.ClientSession(trust_env=True) as session:
                response = await session.get(url=link, data=data, headers=headers, cookies=cookies, timeout=3000)
                soup = BeautifulSoup(await response.text(), "lxml")
        ...
        task = asyncio.create_task(get_page_data(all_cards_links))
